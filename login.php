...
<body class=" 	 bg-gradient-to-r from-slate-500 via-slate-400 to-slate-500">
    <div class="flex justify-center flex-wrap">
        <div class=" container w-100 text-2xl font-bold fixed mt-12">
            <ul class="flex justify-around">
                ...
                <a class="bg-blue-500 rounded-3xl py-2 px-4 border-dashed border-2 border-sky-500" href="login.php">Usuarios</a>
                <a class="bg-blue-500 rounded-3xl py-2 px-4 border-dashed border-2 border-sky-500" href="paises.php">Paises</a>
            </ul>
        </div>

        <div class=" container flex pt-28 px-28 gap-x-8">
    ...
    ">Agregar </button>
                <form class="formulario-visible w-80 bg-white shadow-md rounded px-8 pt-6 pb-8  action=" method="post">
                    ...
                    $query->execute();
                    $results = $query->fetchAll(PDO::FETCH_OBJ);
                    echo "<select class='shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline' name='rol'>\n";
                    if ($query->rowCount() > 0) {
         ...
         values(:nombres,:apellidos,:profesion,:pais,:rol,:fregis,:username,:password)");
          ...
          setTimeout(() => {
           ...
           setTimeout(() => {
            ...
            inner join roles on tbl_personal.roles_idRol=roles.idRol
            inner join pais on tbl_personal.pais_idpais =pais.idpais where id =  :id');
       ...
       let boton= document.getElementById('boton-formulario').classList.add('boton-formulario-padding');
        ...
        <div  class="flex px-32 mx-32 justify-around" style="width: 66rem;">
            <div  style="font-size: 3rem;">
                ...
                echo $obj->cantidad;
                ?>
                <i class="text-center fa-solid fa-user"></i>
